...
//count number sponsor
    ...
    //count number Compitions assigned to sponsor
    public function countCompitionsBySponsor($id)
    {
        return $this->createQueryBuilder('s')
            ->select('count(s.id)')
            ->where('s.id = :id')
    ...
    //count totale of donnation by sponsor
    public function countDonnationBySponsor($id)
    {
        return $this->createQueryBuilder('s')
            ->select('sum(s.donnation)')
            ->where('s.id = :id')
            ->setParameter('id', $id)
            ->getQuery()
            ->getSingleScalarResult();
